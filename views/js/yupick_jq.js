...
var YP_ID_CARRIER = configYupick.yp_carrier_id;
var yupickResultados = '';
var order_opc = configYupick.order_opc;
var isapi = typeof google === 'object';
var gmapsapikey = configYupick.gmapsapikey;
var cashondelivery_modules = configYupick.cashondelivery_modules;
if (!isapi) document.write("<script type='text/javascript' src='https://maps.googleapis.com/maps/api/js?key=" + gmapsapikey + "&sensor=false'><\/script>");
$(document).ready(function() {
    updatePaymenOndelivery();
    if ($('input.delivery_option_radio:checked').val()) {
        var carrieCheck = $('input.delivery_option_radio:checked').val().replace(',', '');
    }
    if (YP_ID_CARRIER == carrieCheck) {
        var position = $("input.delivery_option_radio:checked").closest('.delivery_option');
    ...
    $("#HOOK_PAYMENT").on("click", ".payment_module a", function(event) {
        var payment_selc = $(this).attr('href');
        var carrieCheck = $('input.delivery_option_radio:checked').val().replace(',', '');
        if (order_opc == 1) {
            if (YP_ID_CARRIER == carrieCheck) {
                ...
                if (validade_yupick(event)) {
                    ...
                    if ($('input#cgv:checked').val()) {
                        // valida si el metodo de pago es contrarrembolso   
                        if (validate_cahsondelivery(event, payment_selc)) {
                            // guarda los datos de la oficina para enviarlos
                            if ($('#oficina_yupick_id').val() == "") {
                                alert('No ha seleccionado punto Yupick!');
                                event.preventDefault();
                                return false;
                            }
                            dataYupick();
...
function validate_cahsondelivery(event, payment_selc) {
    $.each(cashondelivery_modules, function(i, val) {
        if (payment_selc.search(val) !== -1) {
            alert('Este metodo de pago no se puede seleccionar con el transportista Yupick!');
            event.preventDefault();
            return false;
        }
    });
...
function updatePaymenOndelivery() {
    if (sessionStorage.yupick_carrier_selecc == '1') {
        $.each(cashondelivery_modules, function(i, val) {
            $('#HOOK_PAYMENT .payment_module a').each(function() {
                if ($(this).attr('href').search(val) !== -1) {
...
function GetYupickPoint() {
    ...
    $.ajax({
        ...
        success: function(JSONObject) {
            $('#oficinas_yupick_content').css("display", "");
            if (JSONObject !== false) {
...
var delivery_option_radio = $('.delivery-option');
    ...
    if (carrier_id === false) {
        $.each(delivery_option_radio, function(i) {
            if ($(this).find('input').is(":checked")) {
                ...
                if (YP_ID_CARRIER == carrier_real_id) {
...
if (is_onepagecheckoutps) {
    Review.placeOrder = (function(event) {
        ...
        return function(event) {
            if (YP_ID_CARRIER == $('.delivery_option_radio:checked').val().replace(',', '') && Payment.validateSelected() && Review._validatePlaceOrder().valid) {
                if (validade_yupick(event)) {
